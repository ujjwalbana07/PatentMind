...
if __name__ == "__main__":
    ...
    try:
        ...
        patent_data = unique_patents

        for patent in patent_data:
            emb = patent.get("embedding")
            if not emb or not isinstance(emb, list) or len(emb) != 768:
                print(f"Skipping '{patent.get('title')}' due to invalid embedding")
                continue
            
    
            # Convert all values to float (just to be safe)
            emb = [float(x) for x in emb]
            print(f"Embedding length for '{patent.get('title')}': {len(emb)}")
            client.index(
                index="patents",
                id=patent.get("id"),
                body={
